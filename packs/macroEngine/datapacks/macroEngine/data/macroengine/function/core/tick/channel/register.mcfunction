# macroengine:core/tick/channel/register — Register or replace a tick channel
# Usage:
# function macroengine:core/tick/channel/register {id:"my_sys",rate:20,offset:0,fn:"my_ns:tick/my_system",enabled:1,condition:""}
#
# Fields:
...
$data modify storage macroengine:engine tick.channels append value {id:"$(id)",enabled:$(enabled)b,rate:$(rate),offset:$(offset),fn:"$(fn)",condition:"$(condition)"}