...
class Uninstall(ConfigField):
    ...
    def render(self, indent=0):
        return ("""tellraw @s {"text":"                                                                                ","color":"%(line_colour)s","strikethrough":true}"""
              + """\ntellraw @s ["",{"text":"◎","color":"dark_red","click_event":{"action":"run_command","command":"/function %(namespace)s:config/uninstall_verif"},"hover_event":{"action":"show_text","value":["",{"text":"Click to Uninstall %(title)s","color":"red"}]}}," Uninstall %(title)s"]"""
              + """\ntellraw @s {"text":"                                                                                \\n","color":"%(line_colour)s","strikethrough":true}\n"""
        ) % self.args
    ...
    def create_files(self, indent=0):
        with open("uninstall_verif.mcfunction" % self.args, "w", encoding="utf-8") as f:
            ...
            print("""tellraw @s ["", {"text":"Confirm uninstall of %(title)s?\\n    "},{"text":"Confirm Uninstall","color":"dark_red","underlined":true,"click_event":{"action":"run_command","command":"/function %(namespace)s:uninstall"},"hover_event":{"action":"show_text","value":["",{"text":"This will remove all settings.","color":"red"}]}}]""" % self.args, file=f)
