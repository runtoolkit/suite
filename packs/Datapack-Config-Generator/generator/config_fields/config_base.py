...
class Foldable(ConfigField):
    ...
    def render(self, indent=0):
        ...
        return ("""# %(label)s"""
                + """\nexecute unless score %(sb_player)s > #zero constants run tellraw @s ["%(indent)s",{"text":"▷ ","color":"red","click_event":{"action":"run_command","command":"/function %(namespace)s:config/%(directory)senable/%(function)s"},"hover_event":{"action":"show_text","value":["",{"text":"Click to enable ","color":"green"},"%(label)s"]}},"%(label)s"]"""
                + """\nexecute if score %(sb_player)s > #zero constants run tellraw @s ["%(indent)s",{"text":"☑","color":"green","bold":true,"click_event":{"action":"run_command","command":"/function %(namespace)s:config/%(directory)sdisable/%(function)s"},"hover_event":{"action":"show_text","value":["",{"text":"Click to disable ","color":"red"},"%(label)s"]}},"%(label)s"]"""
