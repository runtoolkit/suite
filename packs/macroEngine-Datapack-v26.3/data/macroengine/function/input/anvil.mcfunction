...
execute as @a at @s anchored eyes positioned ^ ^ ^1.5 if block ~ ~ ~ minecraft:anvil run execute as @a if items entity @s player.cursor *[minecraft:custom_data={macroengine:{input:1b,inputItem:"anvil"}}] run function macroengine:input/private/anvil_capture
