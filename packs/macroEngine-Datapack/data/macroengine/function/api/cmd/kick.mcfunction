...
$execute if data storage macroengine:engine {sandbox:1b} run return run function macroengine:core/internal/load/gate/request {type:"kick",label:"Kick $(player) — $(reason)",action:"macroengine:core/internal/cmd/kick_apply",args:{player:"$(player)",reason:"$(reason)"}}
