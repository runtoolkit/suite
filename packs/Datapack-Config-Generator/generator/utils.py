...
class SB:
    ...
    def __init__(self, player, scoreboard, default_val = None):
        ...
        # Force fake-player naming convention: prefix with '#' unless already
        # prefixed or referring to an actual player selector (@s, @a, @p, @e, @r,
        # or a UUID-like string). This prevents the fake player name from ever
        # colliding with a real player's username on a live server.
        if not player.startswith("#") and not player.startswith("@"):
            player = "#" + player
        self.player = player
