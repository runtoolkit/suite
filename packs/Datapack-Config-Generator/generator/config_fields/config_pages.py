...
class Page(ConfigField):
    ...
    def create_files(self, indent=0):
        ...
        with open("page_%(page_name)s.mcfunction" % self.args, "w", encoding="utf-8") as f:
            ...
            back = """{"text":" < ", "bold":true, "click_event":{"action":"run_command","command":"/function %(namespace)s:config/%(directory)ssetpage_%(prev_page)s"},"hover_event":{"action":"show_text","value":"Go to previous page"}}""" % self.args
            if self.args["prev_page"] is None: back = """{"text":" < ", "bold":true, "color": "gray"}"""
            next = """{"text":" > ", "bold":true, "click_event":{"action":"run_command","command":"/function %(namespace)s:config/%(directory)ssetpage_%(next_page)s"},"hover_event":{"action":"show_text","value":"Go to next page"}}""" % self.args
            if self.args["next_page"] is None: next = """{"text":" > ", "bold":true, "color": "gray"}"""
            print("""tellraw @s ["\\n", {"text":"Menu ", "bold":true, "click_event":{"action":"run_command","command":"/function %(namespace)s:config/%(directory)ssetpage_%(menu_page)s"},"hover_event":{"action":"show_text","value":"Go to %(menu_title)s"}}, %(back)s, {"text":"%(title)s", "hover_event":{"action":"show_text","value":"Currently on page %(title)s"}}, %(next)s, "\\n"]""" % (self.args | {"back": back, "next": next}), file=f)
...
class MenuPage(Page):
    ...
    def create_files(self, indent=0):
        with open("page_%(menu_page)s.mcfunction" % self.args, "w", encoding="utf-8") as f:
            ...
            for i, p in enumerate(self.pages):
                print("""tellraw @s ["  ", {"text": "%(title)s", "click_event":{"action":"run_command","command":"/function %(namespace)s:config/%(directory)ssetpage_%(page_name)s"},"hover_event":{"action":"show_text","value":"Go to %(title)s"}}]""" % p.args, file=f)
