...
# destroy entities
execute as @e[type=minecraft:interaction,tag=player_action.click_entity,predicate=macroengine_player:v1/equals_uuid] run tp @s ~ -300 ~
execute as @e[type=minecraft:interaction,tag=player_action.click_entity,predicate=macroengine_player:v1/equals_uuid] run kill @s
